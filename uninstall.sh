...
if  [ -z $UI_PID ] ;then
     ...
     read -p ".............If you want delete \"$EXE_FILE\",please close \"$EXE_FILE\" first!!!..........." -n 1
     exit 0
fi

#add by ww
for i in {1..99};do
	if [ -c "/dev/ttyUSB$i" ];then
		read -p "Please unplug the modem before uninstalling the software!" -n 1
		exit 0
	fi
done
...
rm -rf $INSTALL_PATH
rm -rf /usr/local/qt_lib
rm -rf /usr/lib/UniSdk.so
rm -rf /usr/lib/UniCodec.so
rm -rf /usr/lib/UniDevManager.so
rm -rf /usr/lib/UniMMS.so
rm -rf /usr/lib/UniVousb.so
rm -rf /usr/lib/CoreSDK.so
rm -rf /etc/ld.so.conf.d/$FILE_NAM.conf
ldconfig
echo "ldconfig..."
...
TMP_FILE="/usr/bin/$FILE_NAME"
if [ -f $TMP_FILE ];then
	rm -f $TMP_FILE
      echo ..........delete $TMP_FILE ok...........
fi

TMP_FILE="/bin/$FILE_NAME"
if [ -f $TMP_FILE ];then
	rm -f $TMP_FILE
      echo ..........delete $TMP_FILE ok...........
fi
