...
function copy_qt_library
{
	...
	cd $QT_LIBRARY_DIR
	cp UniCodec.so UniDevManager.so UniSdk.so UniVousb.so CoreSDK.so /usr/lib
	rm -rf UniCodec.so UniDevManager.so UniSdk.so UniVousb.so CoreSDK.so
...
function copy_connection_files
{
    ...
    if [ -e $SYS_PAP_SECRETS_FILE -a -f $SYS_PAP_SECRETS_FILE ];then
        chmod 0777 $SYS_PAP_SECRETS_FILE
    fi
	SYS_PAP_SECRETS_FILE="/etc/ppp/chap-secrets"
    if [ -e $SYS_PAP_SECRETS_FILE -a -f $SYS_PAP_SECRETS_FILE ];then
        chmod 0777 $SYS_PAP_SECRETS_FILE
    fi
...
#    fi
    ...
    if [ -f $TMP_FILE ];then
        ...
        cp -f $INSTALL_PATH/wirelessconfig/$TMP_FILE  $TMP_FILE
        chmod 0777 $TMP_FILE
    fi
    TMP_FILE=/etc/ppp/options
    if [ -f $TMP_FILE ];then
...
function setup_udev_rules
{
    ...
    then
        ISSUE_NAME="/etc/issue"
		if [ "$(grep "Ubuntu" $ISSUE_NAME)" ] || [ "$(grep "Fedora" $ISSUE_NAME)" ]; then
			if [ "$(grep "15.04" $ISSUE_NAME)" ] || [ "$(grep "15.10" $ISSUE_NAME)" ] || [ "$(grep "21" $ISSUE_NAME)" ] || [ "$(grep "22" $ISSUE_NAME)" ] || [ "$(grep "23" $ISSUE_NAME)" ]; then
     	        /sbin/udevadm control reload_rules >/dev/null 2>&1
     	        /sbin/udevadm control --reload-rules >/dev/null 2>&1
     	        /sbin/udevadm trigger --subsystem-match=block
     	        echo "the system is ubuntu 15.04,15.10 or fedora 21,22,23"
			else
     	        /sbin/udevadm control --reload-rules >/dev/null 2>&1
     	        /sbin/udevadm trigger subsystem=usb >/dev/null 2>&1
     		    /sbin/udevadm trigger system=block >/dev/null 2>&1
    		    echo "the system is ubuntu or fedora"
   	    fi
		else
                #echo "the system is not ubuntu or fedora "
                #/sbin/udevadm control --reload-rules >/dev/null 2>&1
     	        #/sbin/udevadm trigger subsystem=usb >/dev/null 2>&1
...
function whether_install_driver
{
    ISSUE_NAME_SYS="/etc/issue"
    ISSUE_NAME_SYS_TAIL=""
    if [ "$(grep "Ubuntu" $ISSUE_NAME_SYS)" ] || [ "$(grep "Fedora" $ISSUE_NAME_SYS)" ]; then
	if [ "$(grep "15.04" $ISSUE_NAME_SYS)" ] || 
		[ "$(grep "15.10" $ISSUE_NAME_SYS)" ] || 
		[ "$(grep "16.04" $ISSUE_NAME_SYS)" ] || 
		[ "$(grep "16.10" $ISSUE_NAME_SYS)" ] || 
		[ "$(grep "21" $ISSUE_NAME_SYS)" ] || 
		[ "$(grep "22" $ISSUE_NAME_SYS)" ] || 
		[ "$(grep "23" $ISSUE_NAME_SYS)" ] || 
		[ "$(grep "24" $ISSUE_NAME_SYS)" ]; then
		
		echo $ISSUE_NAME_SYS_TAIL
	else
		install_driver
		add_selinux_rules
		echo $ISSUE_NAME_SYS_TAIL
...
function setup_auto_start_script
{
    ...
    TMP_FILE=$INSTALL_PATH/launch-start
    if [ -f $TMP_FILE ];then
        chown root.root $TMP_FILE
        chmod 0755  $TMP_FILE
        chmod u+s  $TMP_FILE
...
function run_program
{
    if [ $TWO_ON == 'true' -a $rettfp == '0' -a -f $LAST_RUN ];then
        ...
        TEMP_NAME_MEDIA=`who | awk '{print $1}'`
	TEMP_NAME_MEDIA=`echo $TEMP_NAME_MEDIA | awk '{print $1}'`
        echo $TEMP_NAME_MEDIA
        eject /media/$TEMP_NAME_MEDIA/Airtel
	eject /run/media/$TEMP_NAME_MEDIA/Airtel
