...
ISSUE_NAME="/etc/issue"
if [ "$(grep "Ubuntu" $ISSUE_NAME)" ] && [ "$(grep "16.10" $ISSUE_NAME)" ]; then
	$NICE --adjustment=5 $SU $XUSER -c "gnome-terminal --geometry=0x0+10000+10000 -x bash -c 'nohup /bin/launch-start 16.10'" 2>/dev/null &
else
	$NICE --adjustment=5 $SU $XUSER -c "$GUI2LAUNCH &" 2>/dev/null &
fi
